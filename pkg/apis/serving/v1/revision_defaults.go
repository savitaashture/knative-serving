...
func (rs *RevisionSpec) applyDefault(container *corev1.Container, cfg *config.Config) {
	...
	if len(rs.PodSpec.Containers) == 1 {
		...
		// There are multiple containers but default probes will be applied to the container where user specifies PORT not to all containers
...
func (rs *RevisionSpec) applyProbes(container *corev1.Container) {
